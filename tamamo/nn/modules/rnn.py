...
from ...utils.helper import is_cuda_module
...
class StatefulLSTMCell(StatefulBaseCell) :
    ...
    def forward(self, input) :
        ...
        if self.state is None :
            h0 = Variable(tt.FloatTensor(batch, self.rnn_cell.hidden_size).zero_())
            c0 = Variable(tt.FloatTensor(batch, self.rnn_cell.hidden_size).zero_())
            # h0, c0 #
            self.state = (h0, c0)
